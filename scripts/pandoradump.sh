#!/bin/bash
#
# pandoradump.sh - Capture raw Pandora data from a Sonos device
#
# This script connects directly to a Sonos device to dump Pandora-related data
...
# Check for help flag
if [ "$1" = "--help" ] || [ "$1" = "-h" ]; then
    echo "pandoradump.sh - Capture raw Pandora data from a Sonos device"
    echo ""
...
if [ -n "$OUTPUT_FILE" ]; then
    ...
    echo "Capturing Pandora data to $OUTPUT_FILE..." >&2
    node "${SCRIPT_DIR}/pandora-dump-all.cjs" > "$OUTPUT_FILE"
    echo "Output saved to: $OUTPUT_FILE" >&2
else
    # Output to stdout (no status messages)
    node "${SCRIPT_DIR}/pandora-dump-all.cjs"
fi
